      ...
      <div className="relative mx-auto min-h-[760px] max-w-6xl overflow-visible rounded-[30px] border border-[#edf1fb] bg-white shadow-[0_28px_70px_rgba(71,105,175,.12)]">
        ...
        <div className="relative grid min-h-[760px] grid-cols-[126px_minmax(0,1fr)] md:grid-cols-[182px_minmax(0,1fr)]">
          <aside className="z-20 border-r border-[#eef2fa] bg-[linear-gradient(180deg,#fbfcff_0%,#f6f8ff_100%)] px-3 pt-32 md:px-5">
            ...
            <div className="space-y-3">
              {agentes.map((item) => {
                ...
                return <button key={item.id} type="button" onClick={() => onSeleccionar(item.id)} aria-pressed={seleccionado} className={`flex w-full items-center gap-2 rounded-xl p-2 text-left transition-all duration-300 ${seleccionado ? "relative z-30 -translate-y-2 bg-white shadow-[0_15px_30px_rgba(70,106,180,.22)] ring-1 ring-[#e5edff]" : "opacity-60 hover:translate-x-1 hover:bg-white hover:opacity-100"}`}>
          ...
          <main className="relative min-h-[760px] overflow-hidden px-5 pb-16 pt-32 md:px-10 md:pt-36">
            ...
            <div className="absolute inset-x-5 bottom-9 top-28 z-10 rounded-[26px] border border-[#eef2fb] bg-white/92 p-5 shadow-[0_18px_44px_rgba(72,106,174,.09)] md:inset-x-10 md:bottom-14 md:top-36 md:p-8">
              {children}
            </div>

            {alrededor.map((item, indice) => <button key={item.id} type="button" onClick={() => onSeleccionar(item.id)} className={`producto-nodo-esquina producto-nodo-${indice + 1} absolute z-20 grid h-14 w-14 place-items-center rounded-2xl border border-[#edf2fe] bg-white text-[10px] font-black text-[#76a0f4] shadow-[0_12px_28px_rgba(90,126,195,.15)] transition hover:scale-110 focus:outline-none focus:ring-4 focus:ring-[#e1ebff] ${ESQUINAS[indice]}`}>{item.iniciales}<span className="absolute -bottom-4 w-24 truncate text-center text-[8px] font-bold text-[#8d9bb5]">{item.nombre}</span></button>)}

            <div key={agente?.id} className="producto-nucleo-activo absolute left-1/2 top-8 z-30 grid h-[88px] w-[88px] -translate-x-1/2 place-items-center rounded-[25px] bg-[linear-gradient(135deg,#3d74ec,#7dc3ff)] text-lg font-black text-white shadow-[0_22px_45px_rgba(64,119,239,.35)] ring-8 ring-[#edf4ff]">{agente?.iniciales}</div>
