...
export default function PaginaCategoriasProducto() {
  ...
  return <div className="space-y-6">
    ...
    <section id="sec-ranking-producto" className="scroll-mt-24 rounded-[28px] border border-white/90 bg-white/75 p-5 shadow-[0_12px_30px_rgba(44,63,108,.08)]"><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#596bd0]">Detalle operativo</p><h2 className="mt-1 text-xl font-black text-[#253047]">Lectura actual: {dimensionActiva.nombre}</h2><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{categorias.slice(0, 4).map((categoria) => <div key={categoria.nombre} className="rounded-2xl border p-4" style={{ borderColor: `${dimensionActiva.color}33`, background: dimensionActiva.suave }}><p className="truncate text-[10px] font-black uppercase tracking-[.1em]" style={{ color: dimensionActiva.color }}>{categoria.nombre}</p><p className="mt-1 text-xl font-black text-[#29334a]">{fmt(categoria.valor)}</p><p className="mt-1 text-[10px] font-semibold text-[#68758d]">{categoria.productos} productos · {categoria.unidades.toLocaleString("es-GT")} unidades</p></div>)}</div></section>
